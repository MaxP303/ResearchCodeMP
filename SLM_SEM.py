...
y = np.array(gdf['total_ce']).reshape(-1, 1)  # Dependent variable: total_ce
X = np.array(gdf[['total_oi']])  # Independent variable: total_oi

# Spatial Lag Model
print("Spatial Lag Model (SLM) Results:")
slm_model = GM_Lag(y, X, w=knn_weights, name_y='total_ce', name_x=['total_oi'], name_w='knn_weights')
...
sem_model = GM_Error(y, X, w=knn_weights, name_y='total_ce', name_x=['total_oi'], name_w='knn_weights')
